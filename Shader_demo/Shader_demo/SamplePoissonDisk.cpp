...
SamplePoissonDisk::SamplePoissonDisk(TriMesh *mesh)
{
	mesh->need_normals();
	mpMeshVertices = mesh->vertices;
	mpMeshNormals = mesh->normals;
	mpMeshIndices.clear();
	for (size_t i = 0; i < mesh->faces.size(); i++)
	{
		ivec3 f(mesh->faces[i][0], mesh->faces[i][1], mesh->faces[i][2]);
		mpMeshIndices.push_back(f);
	}
}
...
bool SamplePoissonDisk::generateSamples(int numSamples) {
	...
	for(sampleID=0; sampleID<numSamples; sampleID++) {
		...
		Sample sample;
		{
			...
			for(int j=0; j<3; j++) v[j] = (mpMeshVertices)[idx[j]];
			vec3 n[3];
			for (int j = 0; j<3; j++) n[j] = (mpMeshNormals)[idx[j]];
			...
			vec3 p_n =  (v[1]-v[0]) CROSS (v[2]-v[0]);
			//vec3 p_n = n[0] + (n[1] - n[0]) * r1 + (n[2] - n[0]) * r2;
			p_n =  normalize(p_n);
			sample.normal = p_n;
			sample.face_id = faceID;
			sample.weightA = r1;
			sample.weightB = r2;
			sample.mSampleRadius = mSampleRadius;
		}
...
vector<Sample> SamplePoissonDisk::GetSample(int numSamples, bool needCheckVis)
{
	runSampling(numSamples);
	if (needCheckVis == false)
	{
		return mSamples;
	}
